...
namespace FarseerPhysics.Common
{
    ...
    public class Path
    {
        ...
        {
            for (int i = 0; i < ControlPoints.Count; i++)
                ControlPoints[i] = FVector2.Multiply(ControlPoints[i], value);
        }
        ...
        public FVector2 GetPosition(float time)
        {
            FVector2 temp;
            ...
            if (Closed)
            {
                ...
                temp = FVector2.CatmullRom(ControlPoints[p0], ControlPoints[p1], ControlPoints[p2], ControlPoints[p3], lt);
        ...
        public FVector2 GetPositionNormal(float time)
        {
            float offsetTime = time + 0.0001f;

            FVector2 a = GetPosition(time);
            FVector2 b = GetPosition(offsetTime);

            FVector2 output, temp;

            FVector2.Subtract(ref a, ref b, out temp);
...
output = new FVector2();
            ...
            FVector2.Normalize(ref output, out output);
